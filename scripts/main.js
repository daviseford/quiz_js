...
//https://www.gravityhelp.com/documentation/article/quiz-add-on/
function showResults() {
    ...
    if (quiz.checkAnswers()) {
        ...
        if (quizScorePercent >= 75) {
            ...
            $('#reaction').text('You are ready for the Zombie Apocalypse, lock and load!');
        }
        else if (quizScorePercent >= 50) {
            quizResultElement.style.backgroundColor = '#ffc107';
            $('#reaction').text('You would probably do OK after a small nuclear attack.');
        }
        else if (quizScorePercent >= 25) {
            quizResultElement.style.backgroundColor = '#ff9800';
            $('#reaction').text(' Make sure you pack a flashlight when you go camping');
        }
        else if (quizScorePercent >= 0) {
            quizResultElement.style.backgroundColor = '#f44336';
            $('#reaction').text('Shelter in place, you shouldn’t be out.');
        }
